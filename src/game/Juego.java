 ...
 * Clase principal del juego. Se encarga de implementar al jugador, los enemigos y los
 * items en un escenario. Actualiza y repinta los elementos sobre canvas
 *
 */
public class Juego extends Canvas implements Escenario, KeyListener {
	...
	public void inicializarMundo() {
		...
		for (int x = 0; x < 4; x++) {
			...
			enemigo.setCoordenadaX((int) (Math.random()
					* ((Escenario.ANCHO - 50) - (Escenario.ANCHO / 2) + 1) + (Escenario.ANCHO / 2)));
			enemigo.setCoordenadaY((int) (Math.random() * (Escenario.ALTO_JUGABLE - enemigo
					.getAlto())));
			enemigo.setVelocidadY((int) (Math.random() * (2 - 6) - 1));
	...
	public void agregarItems() {
		...
		if (oportunidad == 23) {
			...
			if (oportunidad == 1) {
				...
				capsula.setCoordenadaX((int) (Math.random()
						* ((Escenario.ANCHO - 50) - (Escenario.ANCHO / 2) + 1) + (Escenario.ANCHO / 2)));
				capsula.setCoordenadaY((int) (Math.random()
						* ((Escenario.ALTO_JUGABLE - capsula.getAlto()) - (0) + 1) + 0));
				animados.add(capsula);

			} else if (oportunidad == 2) {
				Comidas comida = new Comidas(this);
				comida.setCoordenadaX((int) (Math.random()
						* ((Escenario.ANCHO - 50) - (Escenario.ANCHO / 2) + 1) + (Escenario.ANCHO / 2)));
				comida.setCoordenadaY((int) (Math.random()
	...
	public void agregarEnemigo() {
		...
		if (opcion == 1) {
			...
			enemigo.setCoordenadaX((int) (Math.random()
					* ((Escenario.ANCHO - 50) - (Escenario.ANCHO / 2) + 1) + (Escenario.ANCHO / 2)));
			enemigo.setCoordenadaY((int) (Math.random()
	...
	/**
	 * Pinta los datos como vida, genkidamas, puntos y aliados
	 * @param g
	 */
	public void pintarDatos(Graphics2D g) {
		...
		g.setFont(new Font("Comic", Font.ITALIC, 20));
		...
		for (int x = 0; x < jugador.getGenkidama(); x++) {
			BufferedImage genki = spriteCache
					.getSprite("items/iconGenkidama.png");
		...
		for (int x = 0; x < jugador.getKrillin(); x++) {
			BufferedImage krillin = spriteCache
					.getSprite("items/iconKrillin.png");
			g.drawImage(krillin, 660 + x * krillin.getWidth(),
					Escenario.ALTO_JUGABLE + 15, this);
		}
		g.fillRect(740, Escenario.ALTO_JUGABLE + 10, 2, 30);
		for (int x = 0; x < jugador.getGohan(); x++) {
			BufferedImage gohan = spriteCache.getSprite("items/iconGohan.png");
			g.drawImage(gohan, 760 + x * gohan.getWidth(),
					Escenario.ALTO_JUGABLE + 15, this);
		}
	...
	public void juegoPerdido() {
		...
		g.setPaint(Color.red);
		g.drawString("JUEGO HECHO POR RODOLFO SEQUEIRA R.", 100, 150);
		strategy.show();
	}
	...
	public void llamarJefePiccolo() {
		...
		ePiccolo.setCoordenadaY((int) (Math.random() * (Escenario.ALTO_JUGABLE - ePiccolo
				.getAlto())));
	...
	public void llamarJefeMonoGigante() {
		...
		eMono.setCoordenadaY((int) (Math.random() * (Escenario.ALTO_JUGABLE - eMono
				.getAlto())));
	...
	public void llamarJefeFreezer() {
		...
		eFreezer.setCoordenadaY((int) (Math.random() * (Escenario.ALTO_JUGABLE - eFreezer
				.getAlto())));
